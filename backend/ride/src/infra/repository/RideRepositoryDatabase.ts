...
export default class RideRepositoryDatabase implements RideRepository {
  ...
  async get(rideId: string): Promise<Ride> {
    const [rideData] = await this.connection.query(
      "select * from cccat12.ride where ride_id = $1",
      [rideId]
    );
    const ride = new Ride(
      rideData.ride_id,
      rideData.passenger_id,
      new Coord(parseFloat(rideData.from_lat), parseFloat(rideData.from_long)),
      new Coord(parseFloat(rideData.to_lat), parseFloat(rideData.to_long)),
      rideData.status,
      rideData.request_date
    );
    ride.driverId = rideData.drive_id;
  ...
  async update(ride: Ride): Promise<void> {
    await this.connection.query(
      "update cccat12.ride set driver_id = $1, status = $2, accept_date = $3, start_date = $4, end_date = $5 where ride_id = $6",
      [
        ride.driverId,
        ride.status.values,
        ride.acceptDate,
